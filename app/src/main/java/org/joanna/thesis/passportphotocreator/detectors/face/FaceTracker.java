...
public class FaceTracker extends Tracker<Face> {
    ...
    public FaceTracker(
            final GraphicOverlay<FaceGraphic> mOverlay,
    ...
    public void onUpdate(
        ...
        List<FacePosition> positions = new ArrayList<FacePosition>();
        if (face.getEulerY() < -ROTATION_THRESHOLD) {
            positions.add(FacePosition.ROTATE_LEFT);
        } else if (face.getEulerY() > ROTATION_THRESHOLD) {
            positions.add(FacePosition.ROTATE_RIGHT);
        }
        if (face.getEulerX() < -ROTATION_THRESHOLD) {
            positions.add(FacePosition.FACE_UP);
        } else if (face.getEulerX() > ROTATION_THRESHOLD) {
            positions.add(FacePosition.FACE_DOWN);
        }
        if (face.getEulerZ() < -ROTATION_THRESHOLD) {
            positions.add(FacePosition.STRAIGHTEN_FROM_LEFT);
        } else if (face.getEulerZ() > ROTATION_THRESHOLD) {
            positions.add(FacePosition.STRAIGHTEN_FROM_RIGHT);
        }
        if (face.getIsLeftEyeOpenProbability() < EYES_OPEN_THRESHOLD) {
            positions.add(FacePosition.LEFT_EYE_OPEN);
        }
        if (face.getIsRightEyeOpenProbability() < EYES_OPEN_THRESHOLD) {
            positions.add(FacePosition.RIGHT_EYE_OPEN);
        }
        if (face.getIsSmilingProbability() > NEUTRAL_FACE_THRESHOLD) {
            positions.add(FacePosition.NEUTRAL_MOUTH);
        }
